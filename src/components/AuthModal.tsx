...
import { Phone, Lock, User, Loader2, X, Globe, MessageCircle } from 'lucide-react';
...
export const AuthModal = ({ onSuccess, onClose }: AuthModalProps) => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Telegram Gateway API response might have request_id in root or inside result
      ...
      if (rid) {
        ...
        throw new Error("Request ID topilmadi");
      }
    } catch (err: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Telegram Gateway check-code response: { "status": "confirmed" } or { "result": { "status": "confirmed" } }
          ...
          {method === 'selection' && (
            ...
            >
              <button 
                ...
                disabled={loading}
                className="w-full glass-button py-5 flex items-center justify-center gap-4 bg-white/5 border-white/10 hover:bg-white hover:text-black transition-all group rounded-2xl"
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest leading-none">Google orqali kirish</span>
              </button>

              <button 
                onClick={() => setMethod('telegram')}
                className="w-full glass-button py-5 flex items-center justify-center gap-4 bg-indigo-600/10 border-indigo-500/20 text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all rounded-2xl"
              >
                <MessageCircle size={20} />
                <span className="text-[10px] font-black uppercase tracking-widest leading-none">Telegram orqali kirish</span>
              </button>
            </motion.div>
          )}
